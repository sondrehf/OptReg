...
typedef RT_MODEL_helicopter_p2_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * helicopter_p2(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(helicopter_p2_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(helicopter_p2_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://helicopter_p2:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: helicopter_p2 -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(helicopter_p2_DW.HILReadEncoderTimebase_Task,
        (t_clock) helicopter_p2_P.HILReadEncoderTimebase_clock, 500.0, -1)
        ;
      if (result == 0) {
