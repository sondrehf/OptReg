...
typedef RT_MODEL_helicopter_without_f_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * helicopter_without_feedback(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(helicopter_without_feedback_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(helicopter_without_feedback_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv,
    "shmem://helicopter_without_feedback:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: helicopter_without_feedback -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      result = hil_task_start
        (helicopter_without_feedback_DW.HILReadEncoderTimebase_Task, (t_clock)
         helicopter_without_feedback_P.HILReadEncoderTimebase_clock, 500.0, -1)
        ;
      if (result == 0) {
